--====================================================
-- 🔥 FIRE HUB | BROOKHAVEN
-- UI REFEITA | PC + MOBILE
-- PARTE 1/?
--====================================================

-- 🔹 Carregar Redz Library (Mobile Friendly)
local Library = loadstring(game:HttpGet("https://pastebin.com/raw/UFtiZ32G"))()
...
-- 🔹 ABAS TESTE (7 abas)
-- =========================================================
local RealInfoTab = Window:MakeTab({
Title = "Info",
Icon = "rbxassetid://15309138473"
})

-- Seção Informações do Hub
RealInfoTab:AddSection({ "Informações do Hub" })
RealInfoTab:AddParagraph({ "Owner / Developer:", "frost230_0_0" })
RealInfoTab:AddParagraph({ "Colaborações:", "" })

-- Função para detectar executor
...
RealInfoTab:AddParagraph({"Executor:", executorName})
RealInfoTab:AddParagraph({"Jogo:", game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name})
RealInfoTab:AddParagraph({"Place ID:", tostring(game.PlaceId)})
RealInfoTab:AddParagraph({"Job ID:", tostring(game.JobId)})

-- Seção Discord
RealInfoTab:AddSection("Discord")
RealInfoTab:AddDiscordInvite({
Name = "Embee Hub | Discord",
Description = "Join server",
Logo = "rbxassetid://126713579020014",
Invite = "https://discord.gg/8r9gD5DN6N"
})
...
local Troll = Window:MakeTab({ Title = "Troll", Icon = "user" })

Troll:AddSection({Name = "Trollar Jogadores"})

Troll:AddSection({Name = "Jogadores"})
...
function DoorFlingPlayer(selectedPlayerName)
    ...
    Troll:AddDropdown({
        Name = "Selecionar Método de Matar",
        Options = {"Ônibus", "Sofá", "Sofá sem ir até o alvo"},
        Default = "Ônibus",
        Callback = function(value)
            methodKill = value
        end
    ...
    Troll:AddButton({
        ...
        Callback = function()
            ...
            if methodKill == "Sofá" then
                KillPlayerCouch()
            elseif methodKill == "Sofá sem ir até o alvo" then
                KillWithCouch()
            else
...
Troll:AddToggle({
    Name = "Roubar Ferramentas do Jogador (Anti-ferramentas)",
    Default = false,
    Callback = function(v)
        local Players = game:GetService("Players")
...
Troll:AddSection({Name = "Método da Casa"})

Troll:AddButton({
    Name = "Ban na Casa - Matar Todos",
    Callback = function()
        local ReplicatedStorage = game:GetService("ReplicatedStorage")
local Players = game:GetService("Players")
local Workspace = game:GetService("Workspace")
...
Troll:AddSection({Name = "Método da Casa"})
Troll:AddButton({
    Name = "Banir da Casa e Matar Todos",
    Callback = function()
        local ReplicatedStorage = game:GetService("ReplicatedStorage")
local Players = game:GetService("Players")
local Workspace = game:GetService("Workspace")
...
Troll:AddSection({Name = "Método Stephanny"})

Troll:AddButton({
  Name = "Stephanny - Irritar",
Description = "Perturba completamente o jogador alvo",
  Callback = function()
            Stephannythrow()
end
})
--[[
Troll:AddButton({
        Name = "[ Irritar com Sofá! ] Stephanny Throw v2",
        Callback = function()
         playerCorrupt()
   end
})
...
Troll:AddSection({Name = "Método Shopping [ Em Breve ]"})
--[[
Troll:AddButton({
...
local kill = Troll:AddSection({Name = "Método Barco"})

Troll:AddButton({
    Name = "Arremessar Barco Todos",
    Callback = function()
        local Player = game.Players.LocalPlayer
    local Character = Player.Character
    local Humanoid = Character:FindFirstChildOfClass("Humanoid")
...
local kill = Troll:AddSection({Name = "Métodos de Clique"})
...
Troll:AddButton({
  Name = "[BETA] Arremessar Portas com Clique",
Description = "Recomendamos chegar perto de outras portas, após ela vir até você, clique no jogador que deseja arremessar",
  Callback = function()
local Players = game:GetService("Players")
local Workspace = game:GetService("Workspace")
local RunService = game:GetService("RunService")
...
Troll:AddButton({
    Name = "Clique Arremessar com Sofá (Ferramenta)",
    Callback = function()
...
local NOME_FERRAMENTA = "Clique Arremessar com Sofá"
...
Troll:AddButton({
    Name = "Clique Matar com Sofá (Ferramenta)",
    Callback = function()
...
local NOME_FERRAMENTA = "Clique Matar com Sofá"
local ferramentaEquipada = false
local nomeAlvo = nil
local loopTP = nil
...
Troll:AddSection({Name = "Método Automático"})

Troll:AddButton({
  Name = "Auto Kart Arremessar",
Description = "Se você bater no chão, resete seu personagem",
  Callback = function()
          AutoKartFling()
end
})
Troll:AddButton({
    Name = "Auto Arremessar Todos",
    Callback = function()
    local Players = game:GetService("Players")
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
    local LocalPlayer = Players.LocalPlayer
...
local kill = Troll:AddSection({Name = "Outros Métodos"})

Troll:AddButton({
    Name = "Matar Todos Ônibus",
    Callback = function()
        local Players = game:GetService("Players")
local Workspace = game:GetService("Workspace")
local RunService = game:GetService("RunService")
...
-- Atualiza opções do dropdown automaticamente
...
-- Função principal de cópia ULTRA OTIMIZADA
local function CopyAvatar()
    ...
    print("Copiando avatar de " .. Target .. "...")

    -- Obter descrições
    local LDesc = LHumanoid:GetAppliedDescription()
    local PDesc = THumanoid:GetAppliedDescription()

    -- FASE 1: RESETAR RÁPIDO
    local itemsToRemove = {}
    
    for _, acc in ipairs(LDesc:GetAccessories(true)) do
        if acc.AssetId and tonumber(acc.AssetId) then
            table.insert(itemsToRemove, tonumber(acc.AssetId))
        end
    end
    
    if tonumber(LDesc.Shirt) then table.insert(itemsToRemove, tonumber(LDesc.Shirt)) end
    if tonumber(LDesc.Pants) then table.insert(itemsToRemove, tonumber(LDesc.Pants)) end
    if tonumber(LDesc.Face) then table.insert(itemsToRemove, tonumber(LDesc.Face)) end
    
    for _, itemId in ipairs(itemsToRemove) do
        spawn(function()
            pcall(function()
                Remotes.Wear:InvokeServer(itemId)
            end)
        end)
    end
    ...
    local bodySuccess = pcall(function()
    ...
    -- FASE 3: ESCALAS E PROPORÇÕES
    pcall(function()
        LHumanoid.BodyDepthScale.Value = PDesc.DepthScale
        LHumanoid.BodyHeightScale.Value = PDesc.HeightScale
        LHumanoid.BodyWidthScale.Value = PDesc.WidthScale
        LHumanoid.HeadScale.Value = PDesc.HeadScale
        LHumanoid.BodyProportionScale.Value = PDesc.ProportionScale
    end)
    
    print("Escalas aplicadas")

    -- FASE 4: ROUPAS
    local clothingItems = {}
    
    if tonumber(PDesc.Shirt) then
        table.insert(clothingItems, {type = "Shirt", id = tonumber(PDesc.Shirt)})
    end
    if tonumber(PDesc.Pants) then
        table.insert(clothingItems, {type = "Pants", id = tonumber(PDesc.Pants)})
    end
    if tonumber(PDesc.Face) then
        table.insert(clothingItems, {type = "Face", id = tonumber(PDesc.Face)})
    end
    ...
    for i, item in ipairs(clothingItems) do
        spawn(function()
            task.wait(i * 0.08)
            pcall(function()
                Remotes.Wear:InvokeServer(item.id)
            end)
        end)
    end
    
    task.wait(0.4)
    print("Roupas copiadas")
    ...
    task.wait(math.max(0.5, #accessories * 0.08))
    print(accessoryCount .. " acessórios copiados")

    -- FASE 6: CORES DO CORPO
    local SkinColor = TPlayer.Character:FindFirstChild("Body Colors")
    if SkinColor then
        pcall(function()
            Remotes.ChangeBodyColor:FireServer(tostring(SkinColor.HeadColor))
        end)
        print("Cores copiadas")
    end

    -- FASE 7: ANIMAÇÕES
    local animations = {
        {PDesc.IdleAnimation, "Idle"},
        {PDesc.WalkAnimation, "Walk"},
        {PDesc.RunAnimation, "Run"},
        {PDesc.JumpAnimation, "Jump"},
        {PDesc.FallAnimation, "Fall"}
    }
    
    for i, anim in ipairs(animations) do
        if tonumber(anim[1]) then
            spawn(function()
                task.wait(i * 0.05)
                pcall(function()
                    Remotes.Wear:InvokeServer(tonumber(anim[1]))
                end)
            end)
        end
    end
    ...
    task.wait(0.3)
    print("Animações copiadas")

    -- FASE 8: NOME E BIO RP
    local Bag = TPlayer:FindFirstChild("PlayersBag")
    if Bag then
        local rpData = {}
        
        if Bag:FindFirstChild("RPName") and Bag.RPName.Value ~= "" then
            table.insert(rpData, function()
                pcall(function()
        ...
        for _, func in ipairs(rpData) do
            spawn(func)
        end
        
        if #rpData > 0 then
            task.wait(0.3)
            print("Nome e Bio RP copiados")
        end
    end

    print("AVATAR TOTALMENTE COPIADO DE: " .. Target)
end

-- Botão principal de cópia
Tab:AddButton({
    Name = "Copiar Avatar Completo",
    Callback = function()
        CopyAvatar()
    end
})

Tab:AddSection({ Name = "Informações" })

Tab:AddParagraph({
    Title = "Selecione um jogador no dropdown\n2. Clique em 'Copiar Avatar Completo'\n3. Aguarde a cópia finalizar",
    Content = "1. Selecione um jogador no dropdown\n2. Clique em 'Atualizar Lista' se necessário\n3. Clique em 'Copiar Avatar Completo'\n4. Aguarde a cópia finalizar"
})
...
local names = {
    {"Frostizinho :3", " Frostizinho :3 "},
    {"Odyzinho :3", " Odyzinho :3 "},
    {"Soareszinho :3", " Soareszinho :3 "},
    {"Embee User !", " Embee User ! "},
    {"Anônimo", " Anônimo "},
    {"Tubers93", " Tubers93 "},
    {"CO0kild", " CO0kild "}
}
...
local CustomShirtId = nil
local CurrentTrack = nil
local RigidTrack = nil

-- ================= SEÇÃO =================
flingsTab:AddSection({"Skybox e Roupa"})
...
local function ChangeShirt(id)
    ...
    shirt.ShirtTemplate = "rbxassetid://" .. tostring(id)
end

-- ================= SKYBOX (DEIVY – FIX GITHUB) =================
...
-- 🔥 FUNÇÃO CORRIGIDA (SEM GetObjects)
local function ActivateSkybox()
    -- ChangeCharacterBody
    ...
    track.TimePosition =
        math.clamp(Settings["Time Position"], 0, 1) * track.Length
...
flingsTab:AddButton({
    ...
    Callback = function()
        ...
        if SkyboxActive then
            ...
            if remotes and remotes:FindFirstChild("ResetCharacterAppearance") then
...
local Tab = Window:MakeTab({"Audio", "music"})

Tab:AddSection({"Som Para Todos"})

Tab:AddParagraph({"Aviso", "Todos no servidor podem ouvir o som"})
...
Tab:AddToggle({
    Name = "Repetir Som Continuamente",
    Default = false,
    Flag = "audio_loop",
    Callback = function(value)
        audioLoop = value
...
Tab:AddButton({"Tocar Som", function()
    playAudio()
end})
...
Tab:AddToggle({
    Name = "Explodir os ouvidos de todo mundo... kkk...",
    Description = "Tocar áudio alto repetidamente para todos",
    Default = false,
    Flag = "audio_spam",
    Callback = function(value)
...
CarTab:AddButton({
    Name = "Remover Todos os Veículos",
    Callback = function()
...
CarTab:AddParagraph({"Aviso", "Recomendamos usar duas vezes para garantir que todos os veículos sejam removidos"})

CarTab:AddButton({
    Name = "Trazer Todos os Veículos",
    Callback = function()
...
CarTab:AddParagraph({"Aviso", "Puxa todos os veículos do servidor para você"})

local SpeedSection = CarTab:AddSection({"Velocidade"})
...
CarTab:AddParagraph({"Aviso", "Ambos turbos/Velocidade não precisam de Gamepass"})

local MusicSection = CarTab:AddSection({"Música"})
...
CarTab:AddTextBox({
    Name = "ID da Música (Gamepass Necessário)",
    PlaceholderText = "Digite Aqui...",
    Callback = function(value)
        playCarMusic(value)
...
CarTab:AddDropdown({
    Name = "Selecionar Música (Gamepass Necessário)",
    Options = musicIds,
    Callback = function(value)
        playCarMusic(value)
        playScooterMusic(value)
...
CarTab:AddParagraph({"Nota", "O script de música funciona em todos os veículos"})

local RGBSection = CarTab:AddSection({"Veículo"})
...
CarTab:AddParagraph({"Nota", "Ativar isso fará seu carro ficar RGB"})
...
local FlySection = CarTab:AddSection({"Outros"})
...
CarTab:AddParagraph({"Nota", "Quando ativado você pode voar com seu carro"})
